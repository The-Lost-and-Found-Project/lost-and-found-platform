...
export default function LfLiveTimingControl({sessionId,startedAt,facilitator,initialMaxMinutes=75}:Props){
 ...
 return <div className="fixed left-2 right-2 top-[calc(4.75rem+env(safe-area-inset-top))] z-40 sm:left-auto sm:right-4 sm:top-20 sm:w-[min(22rem,calc(100vw-2rem))]">
  <div className="rounded-2xl border border-[#c8a96b]/30 bg-[#091625]/95 p-3 text-white shadow-2xl backdrop-blur">
   <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#d9bd7d]">Study timing</p><p className="mt-1 truncate text-xs font-black sm:text-sm">{remaining} min remaining · {maxMinutes} min window</p></div>{maxMinutes===75&&elapsedMinutes>=60&&<button onClick={()=>setOpen(v=>!v)} className="shrink-0 rounded-xl bg-[#d9bd7d] px-3 py-2 text-xs font-black text-[#07111f]">+15 min</button>}</div>
