...
export default function TriviaClient({userId,categories,bestScores:initialBestScores,initialMode="browse",initialCategoryId=null}:Props){
 ...
 if(screen==="quiz"&&question){const answered=selectedChoice!==null;const isLast=questionIndex===questions.length-1;return <div className="mt-6">
  ...
  <section className="lfp-card mt-5 p-6 sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[11px] font-black uppercase tracking-[.16em] text-blue-700">{category.name}</p>{question.difficulty&&<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black capitalize text-slate-600">{question.difficulty}</span>}</div><h3 className="mt-3 text-2xl font-black leading-tight text-slate-950 sm:text-3xl">{question.question}</h3><div className="mt-6 grid gap-3">{question.choices.map(choice=>{const correct=choice===question.correct;const selected=choice===selectedChoice;let cls="border-slate-200 bg-white";if(answered&&correct)cls="border-emerald-300 bg-emerald-50";else if(answered&&selected&&!correct)cls="border-rose-300 bg-rose-50";return <button key={choice} disabled={answered} onClick={()=>selectChoice(choice)} className={`rounded-2xl border px-4 py-4 text-left font-bold text-slate-800 transition ${cls}`}>{choice}</button>})}</div>
   {answered&&<div className="mt-6 rounded-[1.4rem] bg-blue-50 p-5"><p className="text-[11px] font-black uppercase tracking-[.16em] text-blue-700">After the Answer · {question.ref}</p><p className="mt-2 leading-7 text-slate-700">{question.note}</p>{category.id==="language-insights"&&<p className="mt-3 text-sm font-bold leading-6 text-violet-800">Language guardrail: semantic range is not a menu of simultaneous meanings. Read the word in its sentence, grammar, author, genre, and wider context.</p>}<div className="mt-4 flex flex-wrap gap-3"><a href="/memory" className="font-black text-blue-700">Remember Scripture →</a><a href="/library?type=study" className="font-black text-blue-700">Understand with an L&F Study →</a><a href={`/auth/emmaus?next=${encodeURIComponent("/study/bible")}`} className="font-black text-blue-700">Dig deeper in Emmaus →</a></div></div>}
 ...
 const pct=questions.length?score/questions.length:0;return <div className="mt-6 lfp-card p-8 text-center"><p className="lfp-eyebrow">{daily?"Daily Challenge complete":category.name}</p><div className="mx-auto mt-5 flex h-24 w-24 items-center justify-center rounded-full bg-blue-50 text-3xl font-black text-blue-700">{score}/{questions.length}</div><h3 className="mt-5 text-3xl font-black text-slate-950">{pct===1?"Perfect round.":pct>=.7?"Strong work.":"Keep building."}</h3><p className="mx-auto mt-3 max-w-xl leading-7 text-slate-600">The score closes the round; it does not close the lesson. Move one step further so the truth becomes something you remember, understand, and apply.</p><div className="mx-auto mt-6 grid max-w-3xl gap-3 sm:grid-cols-3"><a href="/memory" className="rounded-2xl bg-blue-50 p-4 text-left"><span className="text-[10px] font-black uppercase tracking-[.13em] text-blue-700">Remember</span><span className="mt-1 block font-black text-slate-950">Practice a memory verse →</span></a><a href="/library?type=study" className="rounded-2xl bg-emerald-50 p-4 text-left"><span className="text-[10px] font-black uppercase tracking-[.13em] text-emerald-700">Understand</span><span className="mt-1 block font-black text-slate-950">Open an L&F Study →</span></a><a href="/auth/emmaus?next=/study" className="rounded-2xl bg-violet-50 p-4 text-left"><span className="text-[10px] font-black uppercase tracking-[.13em] text-violet-700">Dig deeper</span><span className="mt-1 block font-black text-slate-950">Continue in Emmaus →</span></a></div><div className="mt-6 flex flex-wrap justify-center gap-3"><button onClick={()=>loadCategory(category.id,{daily,difficulty})} className="lfp-button lfp-button-primary">Play again</button><button onClick={reset} className="lfp-button lfp-button-secondary">Choose another challenge</button></div></div>
}
