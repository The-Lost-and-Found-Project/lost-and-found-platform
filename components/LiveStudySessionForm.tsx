...
export default function LiveStudySessionForm({ studies, facilitators, participants, ministries, groups }: { studies: Study[]; facilitators: Person[]; participants: Person[]; ministries: Ministry[]; groups: Group[] }) {
  ...
  return <form id="live-study-session-form" action={submit} className="lfp-card mt-6 grid gap-4 p-6 sm:grid-cols-2">
    ...
    <label className="text-sm font-bold">Study group<select name="group_id" value={selectedGroup} onChange={e=>chooseGroup(e.target.value)} className="mt-1 w-full rounded-xl border p-3"><option value="">No saved group</option>{groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">Choosing a group preselects its active roster. You can still add or remove people for this session.</span></label>
    <label className="text-sm font-bold">Ministry<select required name="ministry_slug" defaultValue={group?.ministry_slug||""} key={group?.id||"none"} className="mt-1 w-full rounded-xl border p-3"><option value="">Choose ministry</option>{ministries.map(m=><option key={m.slug} value={m.slug}>{m.title}</option>)}</select></label>
