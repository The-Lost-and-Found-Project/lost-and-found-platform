...
export default function LiveStudySessionForm({studies,facilitators,participants,ministries,groups}:{studies:Study[];facilitators:Person[];participants:Person[];ministries:Ministry[];groups:Group[]}){
 const[pending,setPending]=useState(false);const[message,setMessage]=useState<string|null>(null);const[selectedGroup,setSelectedGroup]=useState("");const[checked,setChecked]=useState<Set<string>>(new Set());const[selectedFacilitator,setSelectedFacilitator]=useState("");
 ...
 return <form id="live-study-session-form" action={submit} className="lfp-card mt-6 grid gap-4 p-6 sm:grid-cols-2">
  ...
  <label className="text-sm font-bold">Ministry<select required name="ministry_slug" value={group?.ministry_slug||""} onChange={()=>{}} className="mt-1 w-full rounded-xl border p-3"><option value="">Choose ministry</option>{ministries.map(m=><option key={m.slug} value={m.slug}>{m.title}</option>)}</select></label>
