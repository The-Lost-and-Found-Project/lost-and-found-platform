  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
      <aside className="self-start rounded-3xl border border-gray-200 bg-white p-5 shadow-sm xl:sticky xl:top-6">
        ...
        <p className="mt-2 text-sm leading-6 text-gray-600">Choose a book and chapter, then continue into the Scripture Importer.</p>
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-4xl font-bold text-gray-950">{selectedBook.name}</h2>
            <p className="mt-2 text-gray-600">Select a chapter to prepare a Scripture import.</p>
          </div>
        ...
        <div className="mt-8 rounded-3xl border border-indigo-200 bg-indigo-50/70 p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-700">Selected passage</p>
          <h3 className="mt-2 text-3xl font-bold text-gray-950">{selectedBook.name} {selectedChapter}</h3>
          <p className="mt-3 max-w-2xl leading-7 text-gray-600">
            Automatic chapter text retrieval is not connected yet. Continue to the importer to paste verified {translation} text, preview the verses, and create missing graph nodes.
          </p>
