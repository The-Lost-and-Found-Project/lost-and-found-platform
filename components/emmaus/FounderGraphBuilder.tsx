...
export default function FounderGraphBuilder() {
  ...
  async function endPointer() {
    if (dragging) {
      ...
      setDragging(null);
      if (node && position) await supabase.from("emmaus_graph_nodes").update({ metadata: { ...(node.metadata ?? {}), canvas_x: position.x, canvas_y: position.y } }).eq("id", node.id);
    }
    setPanning(null);
  }
  ...
  async function autoLayout() {
    const arranged = Object.fromEntries(visibleNodes.map((node, index) => [node.id, { x: 60 + (index % 4) * 270, y: 60 + Math.floor(index / 4) * 170 }])) as Record<string, Position>;
    setPositions((current) => ({ ...current, ...arranged })); setMessage("Saving layout...");
    await Promise.all(visibleNodes.map((node) => supabase.from("emmaus_graph_nodes").update({ metadata: { ...(node.metadata ?? {}), canvas_x: arranged[node.id].x, canvas_y: arranged[node.id].y } }).eq("id", node.id)));
  ...
  return <div className="grid gap-5 xl:grid-cols-[280px_1fr_340px]">
    ...
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl">
      <div className="space-y-3 border-b border-slate-200 bg-slate-950 px-5 py-4 text-white">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-amber-300">Visual Graph Builder</p><p className="mt-1 text-sm text-indigo-100/70">{visibleNodes.length} nodes · {visibleEdges.length} visible relationships · drag empty space to pan</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!selectedNodeId} onClick={() => setConnectSource(selectedNodeId)} className="rounded-full bg-amber-300 px-4 py-2 text-sm font-black text-slate-950 disabled:opacity-40">{connectSource ? "Select target…" : "Connect selected"}</button><button type="button" onClick={() => void autoLayout()} className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-black">Auto-layout</button></div></div>
