...
test("ministry spaces expose the common operating navigation",()=>{const page=read("app/ministries/[slug]/page.tsx");for(const label of ["Home","Updates","Gatherings","Resources","People"])assert.match(page,new RegExp(`>${label}<`));assert.match(page,/All L&F gatherings/);assert.match(page,/One L&F family/);assert.match(page,/Distinct space\. Shared community\./);});
