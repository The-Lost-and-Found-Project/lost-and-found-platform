...
export default async function LibraryPage({searchParams}:{searchParams:Promise<{type?:string}>}){
 const {type}=await searchParams; const activeType=TYPES.some(t=>t.key===type)?type:undefined;
 ...
 const visible=activeType?(items??[]).filter((x:any)=>x.content_type===activeType):(items??[]); const featured=visible.filter((x:any)=>x.is_featured).slice(0,3); const rest=visible.filter((x:any)=>!x.is_featured);
 return <main className="lfp-page pb-24">
  ...
  <div className="lfp-shell py-8 sm:py-12">{!user&&<div className="mb-8 rounded-[1.5rem] border border-sky-100 bg-sky-50 p-5 text-sm leading-6 text-slate-700"><strong>Browse freely.</strong> Sign in only when you want to save a resource or track progress.</div>}
   <section className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">{TYPES.map(t=><Link key={t.key} href={`/library?type=${t.key}#${t.key}`} className={"lfp-card p-4 "+(activeType===t.key?"ring-2 ring-blue-500":"")}><span className="text-xl text-blue-700">{t.icon}</span><h2 className="mt-3 font-black text-slate-950">{t.label}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{t.copy}</p></Link>)}</section>
   ...
   {TYPES.filter(type=>!activeType||type.key===activeType).map(type=>{const group=rest.filter((x:any)=>x.content_type===type.key); if(!group.length)return null; return <section key={type.key} id={type.key} className="mt-12 scroll-mt-24"><div className="flex items-end justify-between gap-4"><div><p className="lfp-eyebrow">{type.label}</p><h2 className="mt-2 text-3xl font-black text-slate-950">{type.copy}</h2></div><span className="text-sm font-bold text-slate-400">{group.length} available</span></div><div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{group.map((item:any)=><CatalogCard key={item.id} item={item}/>)}</div></section>})}
   {!visible.length&&<section className="mt-12 rounded-[2rem] border border-dashed border-slate-300 bg-white/70 p-8 text-center"><p className="text-3xl">▦</p><h2 className="mt-3 text-2xl font-black text-slate-950">The new library is ready for content.</h2><p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-600">Once the catalog migration is applied, your original studies, devotions, trivia, audio, video, and approved resources will appear here without requiring a code change.</p></section>}
  </div>
 </main>;
}

function CatalogCard({item}:{item:any}){const href=item.provenance==="emmaus"?"/auth/emmaus?next=/study":item.external_url||`/library/${item.slug}`; const external=Boolean(item.external_url)&&item.provenance!=="emmaus"; return external?<a href={href} target="_blank" rel="noopener noreferrer" className="lfp-card group p-6"><CardBody item={item}/></a>:<Link href={href} className="lfp-card group p-6"><CardBody item={item}/></Link>}
