...
export default async function TriviaPage({searchParams}:{searchParams:Promise<SearchParams>}){
 ...
 const initialMode=params.mode==="daily"?"daily":params.category?"category":"browse";
 const requestedCategory=params.category&&categories.some(c=>c.id===params.category)?params.category:null;
 return <main className="lfp-page pb-24"><section className="relative overflow-hidden"><div aria-hidden className="lfp-grid absolute inset-0"/><div className="lfp-shell relative py-10 sm:py-14"><p className="lfp-eyebrow">Member Learning Lab</p><h1 className="mt-3 max-w-4xl text-4xl font-black tracking-[-.045em] text-slate-950 sm:text-6xl">Bible learning that helps you <span className="lfp-gradient-text">learn the Word</span>, {firstName}.</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">Questions are drawn from the approved L&F bank. Every answer points back to Scripture and includes a teaching insight—not just a score.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/learn" className="lfp-button lfp-button-secondary">← Learning Lab</Link><Link href="/trivia?mode=daily" className="lfp-button lfp-button-secondary">Daily Challenge</Link><Link href="/trivia?category=language-insights" className="lfp-button lfp-button-secondary">Language Insights</Link><Link href="/memory" className="lfp-button lfp-button-secondary">Memory Verses</Link></div><div className="mt-8 grid grid-cols-3 gap-3"><Stat value={String(categories.filter(c=>c.approvedCount>0).length)} label="Categories"/><Stat value={String(total)} label="Questions"/><Stat value={String(Object.keys(bestScores).length)} label="Attempted"/></div></div></section><div className="lfp-shell py-8"><section className="lfp-card p-5 sm:p-8"><div className="max-w-3xl"><p className="lfp-eyebrow">Choose a challenge</p><h2 className="mt-2 text-3xl font-black text-slate-950">Play. Learn. Dig deeper.</h2><p className="mt-3 leading-7 text-slate-600">Missed questions become learning moments. Language Insights uses context and grammar responsibly: context determines the semantic sense rather than treating every dictionary gloss as the meaning of a word.</p></div><TriviaClient userId={user.id} categories={categories} bestScores={bestScores} initialMode={initialMode} initialCategoryId={requestedCategory}/></section></div></main>
}
