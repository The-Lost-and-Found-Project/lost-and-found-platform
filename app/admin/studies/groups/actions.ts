...
export async function saveGroupMembers(f:FormData){await requireAdmin();const db=createAdminClient();const groupId=text(f,"group_id");if(!groupId)throw new Error("Missing group.");const memberIds=Array.from(new Set(f.getAll("member_user_id").map(String).filter(Boolean)));const facilitatorIds=new Set(f.getAll("facilitator_user_id").map(String));await db.from("study_group_members").delete().eq("group_id",groupId);if(memberIds.length){const{error}=await db.from("study_group_members").insert(memberIds.map(user_id=>({group_id:groupId,user_id,group_role:facilitatorIds.has(user_id)?"facilitator":"member",membership_status:"active"})));if(error)throw new Error(error.message);}revalidatePath("/admin/studies/groups");revalidatePath("/admin/studies");}
