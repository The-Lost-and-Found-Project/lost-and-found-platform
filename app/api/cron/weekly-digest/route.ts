...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const newMembersHtml =
      memberCount === 0
        ? `<p style="color:#555;">No new members joined this week.</p>`
        : (newMembers ?? [])
            .map((m) => {
              const joined = m.created_at
                ? dateFmt(new Date(m.created_at))
                : "";
              return `
                <div style="border-left:3px solid #10b981;margin:12px 0;padding:8px 14px;background:#f9fafb;border-radius:4px;">
                  <p style="margin:0 0 4px 0;color:#111;">${
                    m.full_name ?? "New member"
                  }</p>
                  <p style="margin:0;font-size:13px;color:#666;">
                    ${m.email ?? "No email on file"}${joined ? ` · Joined ${joined}` : ""}
                  </p>
                </div>
              `;
            })
