...
export async function POST(_request:NextRequest,{params}:{params:Promise<{sessionId:string}>}){
 try{
  ...
  if(!isDailyConfigured())return NextResponse.json({configured:false,sessionId,title:study?.title||"L&F Live Study",subtitle:study?.subtitle||null,slides:Array.isArray(study?.slides)?study.slides:[],facilitator,supervisor,message:"L&F Live is built and waiting for the Daily API key. Google Meet remains available as fallback."});
  ...
  return NextResponse.json({configured:true,sessionId,roomUrl:room.url,token,title:study?.title||"L&F Live Study",subtitle:study?.subtitle||null,slides:Array.isArray(study?.slides)?study.slides:[],facilitator,supervisor});
