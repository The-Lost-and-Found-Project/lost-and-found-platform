...
begin
  ...
  loop
    ...
    if scripture_id is null then
      ...
      update public.emmaus_scripture_nodes set
        text_content=verse_text,reference_label=v_reference_label,status=record_status,
        source_batch_id=batch_id,updated_at=now()
    ...
    update public.emmaus_graph_nodes set
      summary=verse_text,title=v_reference_label,subtitle=p_translation,
      scripture_reference=v_reference_label,status=record_status,
    ...
    values(graph_node_id,chapter_node_id,'part_of',v_reference_label||' is part of '||bible_book.name||' '||p_chapter||'.',record_status,jsonb_build_object('structural',true,'translation',p_translation))
    ...
    if previous_graph_node_id is not null then
      insert into public.emmaus_graph_edges(source_node_id,target_node_id,relationship_key,explanation,status,metadata)
      values(previous_graph_node_id,graph_node_id,'recommended_next','Continue reading to the next verse.',record_status,jsonb_build_object('sequence',true,'translation',p_translation))
      on conflict(source_node_id,target_node_id,relationship_key) do update set
    ...
    if p_queue_enrichment then
      ...
      select scripture_id,enrichment_type,priority,jsonb_build_object('book_key',p_book_key,'chapter',p_chapter,'verse',verse_number,'translation',p_translation)
      from (values('people',40),('places',35),('events',45),('themes',60),('questions',65),('language',30),('cross_references',55),('historical_context',25)) q(enrichment_type,priority)
  ...
  update public.emmaus_scripture_import_batches set
    imported_verse_count=imported_count+updated_count,status='verified',completed_at=now(),verified_at=now()
  where id=batch_id;

  return jsonb_build_object(
    'batch_id',batch_id,'translation',p_translation,'book_key',p_book_key,'chapter',p_chapter,
    'inserted',imported_count,'updated',updated_count,'enrichment_tasks_created',queued_count,
    'chapter_graph_node_id',chapter_node_id,'status',record_status
  );
exception when others then
  if batch_id is not null then
    update public.emmaus_scripture_import_batches set status='failed',error_message=sqlerrm,completed_at=now() where id=batch_id;
  end if;
  raise;
end;
$$;
