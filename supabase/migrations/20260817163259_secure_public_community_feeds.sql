...
-- The production Community tables predate this repository's tracked migration
-- baseline. Fresh CI databases intentionally do not recreate that historical
-- schema, so apply this hardening only where all source tables already exist.
do $migration$
begin
  if to_regclass('public.prayer_requests') is null
    ...
    raise notice 'Skipping public Community feed hardening: legacy source tables are not present.';
    return;
  end if;
  ...
  -- Older production environments already have this lifecycle flag. Add it
  -- only when missing so the secured prayer projection remains consistent.
